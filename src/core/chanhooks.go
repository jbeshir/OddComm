...
var hookChanAdd = make(map[string][]func(interface{}, *Channel))
var hookChanDataChanges = make(map[string][]func(interface{}, *User, *Channel, []DataChange, []string))
var hookChanUserJoin = make(map[string][]func(interface{}, *Channel, []*User))
var hookChanUserRemove = make(map[string][]func(interface{}, *User, *User, *Channel, string))
var hookChanDelete = make(map[string][]func(interface{}, *Channel))

var hookChanDataChange = make(map[string]map[string][]func(interface{}, *User, *Channel, string, string))
var hookChanMessage = make(map[string]map[string][]func(interface{}, *User, *Channel, []byte))


// HookChanAdd adds a hook called whenever a new channel is created.
// t is the type of channel to hook. "" is default.
func HookChanAdd(t string, f func(interface{}, *Channel)) {
...
func HookChanDataChange(t, name string, f func(interface{}, *User, *Channel, string, string)) {
	if hookChanDataChange[t] == nil {
		hookChanDataChange[t] = make(map[string][]func(interface{}, *User, *Channel, string, string))
	}
...
func HookChanDataChanges(t string, f func(interface{}, *User, *Channel, []DataChange, []string)) {
...
func HookChanUserJoin(t string, f func(interface{}, *Channel, []*User)) {
...
func HookChanUserRemove(t string, f func(interface{}, *User, *User, *Channel, string)) {
	hookChanUserRemove[t] = append(hookChanUserRemove[t], f)
}

// HookChanDelete adds a hook called whenever a channel is deleted.
// t is the type of channel to hook. "" is default.
func HookChanDelete(t string, f func(interface{}, *Channel)) {
...
func HookChanMessage(chant, msgt string, f func(interface{}, *User, *Channel, []byte)) {
	if hookChanMessage[chant] == nil {
		hookChanMessage[chant] = make(map[string][]func(interface{}, *User, *Channel, []byte))
	}
	hookChanMessage[chant][msgt] = append(hookChanMessage[chant][msgt], f)
}


func runChanAddHooks(origin interface{}, t string, ch *Channel) {
	for _, f := range hookChanAdd[t] {
		f(origin, ch)
	}
}

func runChanUserJoinHooks(origin interface{}, t string, ch *Channel, users []*User) {
	for _, f := range hookChanUserJoin[t] {
		f(origin, ch, users)
	}
}

func runChanUserRemoveHooks(origin interface{}, t string, source *User, target *User, ch *Channel, message string) {
	for _, f := range hookChanUserRemove[t] {
		f(origin, source, target, ch, message)
	}
}

func runChanDataChangeHooks(origin interface{}, t string, source *User, ch *Channel, name, oldvalue, newvalue string) {
	if hookChanDataChange[t] == nil {
		return
	}
	for _, f := range hookChanDataChange[t][name] {
		f(origin, source, ch, oldvalue, newvalue)
	}
}

func runChanDataChangesHooks(origin interface{}, t string, source *User, ch *Channel, c []DataChange, o []string) {
	for _, f := range hookChanDataChanges[t] {
		f(origin, source, ch, c, o)
	}
}

func runChanMessageHooks(origin interface{}, t, msgt string, source *User, ch *Channel, message []byte) {
	if hookChanMessage[t] == nil {
		return
	}
	for _, f := range hookChanMessage[t][msgt] {
		f(origin, source, ch, message)
	}
}
