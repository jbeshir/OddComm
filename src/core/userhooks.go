...
var hookUserRegister []func(interface{}, *User)

var hookUserNickChange struct {
	all    []func(interface{}, *User, string, string, int64)
	regged []func(interface{}, *User, string, string, int64)
}

var hookUserDataChanges struct {
	all    []func(interface{}, *User, *User, []DataChange, []string)
	regged []func(interface{}, *User, *User, []DataChange, []string)
}

var hookUserDelete struct {
	all    []func(interface{}, *User, *User, string)
	regged []func(interface{}, *User, *User, string)
}

type hookDataChangeType struct {
        all    []func(interface{}, *User, *User, string, string)
	regged []func(interface{}, *User, *User, string, string)
}
var hookDataChange = make(map[string]hookDataChangeType)

var hookUserMessage = make(map[string][]func(interface{}, *User, *User, []byte))
...
func HookUserRegister(f func(interface{}, *User)) {
...
func HookUserNickChange(f func(interface{}, *User, string, string, int64), unregged bool) {
	if unregged {
...
func HookUserDataChange(name string, f func(interface{}, *User, *User, string, string), unregged bool) {
...
func HookUserDataChanges(f func(interface{}, *User, *User, []DataChange, []string), unregged bool) {
	if unregged {
...
func HookUserMessage(t string, f func(interface{}, *User, *User, []byte)) {
...
// If unregged is false, it is not called for unregistered users.
func HookUserDelete(f func(interface{}, *User, *User, string), unregged bool) {
	if unregged {
...
func runUserRegisterHooks(origin interface{}, u *User) {
	for _, f := range hookUserRegister {
		f(origin, u)
	}
}

func runUserNickChangeHooks(origin interface{}, u *User, oldnick, newnick string, ts int64) {
	for _, f := range hookUserNickChange.all {
		f(origin, u, oldnick, newnick, ts)
	}
...
func runUserDataChangesHooks(origin interface{}, source, target *User, changes []DataChange, old []string) {
	for _, f := range hookUserDataChanges.all {
		f(origin, source, target, changes, old)
	}
...
func runUserDeleteHooks(origin interface{}, source, target *User, message string, regged bool) {
	for _, f := range hookUserDelete.all {
		f(origin, source, target, message)
	}
...
func runUserDataChangeHooks(origin interface{}, source, target *User, name, oldvalue, newvalue string) {
	for _, f := range hookDataChange[name].all {
		f(origin, source, target, oldvalue, newvalue)
	}
...
func runUserMessageHooks(origin interface{}, source, target *User, message []byte, t string) {
