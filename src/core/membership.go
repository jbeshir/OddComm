...
func (m *Membership) SetData(origin interface{}, source *User, name, value string) {
	var oldvalue string
	...
	hookRunner <- func() {
		runMemberDataChangeHooks(origin, m.c.Type(), source, m, name, oldvalue, value)
		runChanDataChangesHooks(origin, m.c.Type(), source, m.c, []DataChange{c}, []string{oldvalue})
	}
...
// source may be nil, in which case the metadata is set by the server.
func (m *Membership) SetDataList(origin interface{}, source *User, changes []DataChange) {
	...
	hookRunner <- func() {
		for i, it := range changes {
			runMemberDataChangeHooks(origin, m.c.Type(), source, m, it.Name, old[i], it.Data)
		}
		runChanDataChangesHooks(origin, m.c.Type(), source, m.c, done, old)
	}
...
func (m *Membership) Remove(origin interface{}, source *User, message string) {
	...
	hookRunner <- func() {
		runChanUserRemoveHooks(origin, m.c.t, source, m.u, m.c, message)
	}
